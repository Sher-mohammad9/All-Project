...
exports.signupUser = asyncErrorHandler(async (req, resp, next) => {
  ...
  resp
    .cookie("token", token)
    .render("home", { user: NEW_USER, message: "Successfully Signup" });
});
...
exports.logInUser = asyncErrorHandler(async (req, resp, next) => {
  ...
  if (USER_DATA) {
    ...
    if (checkPassword) {
      ...
      return resp
        .cookie("token", token)
        .render("home", { user : USER_DATA, message : "Successfully Login" });
    } else {
...
exports.removeProfilePhoto = asyncErrorHandler(async(req, resp, next)=>{
    let USER_DATA = await userModel.findById(req.params.id);
    if(!USER_DATA){
        const error = new CustomError("User Not Found", 404);
        return next(error);
    }
    USER_DATA = await userModel.findByIdAndUpdate(req.params.id, {$set : {profileImage : "/Images/default.png"}}, {new : true});
    resp.render("home", {user : USER_DATA});
})
